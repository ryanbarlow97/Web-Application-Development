...
<x-app-layout>
    ...
    @foreach ($user->likedComments as $likedComments)
    <table class = "table1 space">
    <tr>
        <td>
            <a href="{{route('users.show', ['id' => $likedComments->user->id])}}"><img src="{{ $likedComments->user->profile_picture}}" style="width:42px;height:42px;"></a>
        </td>
        <td>
            {{ $likedComments->content}}
        </td>
    </tr>
    <table>
    @endforeach
