...
<x-app-layout>
    ...
    @foreach ($user->likedPosts as $likedPosts)
    <table class = "table1 space">
    <tr>
        <td>
            <a href="{{route('users.show', ['id' => $likedPosts->user->id])}}"><img src="{{ $likedPosts->user->profile_picture}}" style="width:42px;height:42px;"></a>
        </td>
        <td>
            {{ $likedPosts->content}}
        </td>
